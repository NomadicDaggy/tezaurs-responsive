...
function updateMorphotable(data, paradigm, pluralEntryWord, secondThirdConj, selector) {
	...
	if (inflections) {
		...
		$("#toggle_Morphology", selector).click(function() { // TODO: jāvispārina kā atsevišķa f-cija
...
function loadContent(doc, word) {
    ...
    $("#mainContent").load(doc + " #doc", function() {
        ...
        if (word != null) {
            ...
            $('.sv_Section', '.sv_Idioms')
                .append(' <img src="img/expand.svg" id="toggle_Idioms" class="expand_SVG" width="14" height="14" alt="+"/>');

            $('.sv_Items', '.sv_Idioms').hide();
            $("#toggle_Idioms").click(function() { // TODO: jāvispārina kā atsevišķa f-cija
            ...
            */
				...
				for (i = 0; i < paradigms.length; i++) {
					...
					if ($(selector).length == 0) {
						$('#morphology', '#' + docId).html(
								$('#morphology', '#' + docId).html()
								+ '<div id="' + localID +'" class="morphology"></div>');
					}

					if (paradigm > 0) {
						var url = 'http://api.tezaurs.lv/v1/inflections/' + word;
						 ...
						 */
						if (typeof stem1 !== "undefined" || typeof stem2 !== "undefined" || typeof stem3 !== "undefined") {
							if (typeof stem1 !== "undefined") url = url + '&stem1=' + stem1;
							else url = url + '&stem1=----';
							if (typeof stem2 !== "undefined" && stem2 != 'apnull' && stem2 != 'pienull') url = url + '&stem2=' + stem2;
							else url = url + '&stem2=----';
						...
						(function(selector, paradigm) {
							...
							var morph_call = $.get(url).done(function(data) {
								...
								if (calls_remaining<1)
									updateMorphotable(inflections[selector], paradigm, pluralEntryWord, secondThirdConj, selector) 
            ...
            // Examples - BEGIN
            $.get('http://api.tezaurs.lv/v1/examples/' + word, function(data) {
                if (data && data.length > 0) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        example_html += '<small>(<i>' + data[i]['title'] + '</i>' + sourceauthor + ')</small>';
                        example_html += '</div>';
                    };

                    example_html += "<div class='disclaimer'>Piemēri ir atlasīti automātiski un var būt neprecīzi.</div>";

                    if (more) {
                        var bonito = "http://bonito.korpuss.lv/bonito/run.cgi/first?corpname=LUMII_lidzsvarotais&iquery=" + word;
                        example_html += '<div class="example"><a href="' + bonito + '" target="_blank">Vairāk...</a></div>';
                    }
                    ...
                    $('#examples').html(
                        ...
                        + '<div class="items">' + example_html + '</div>');

                    $(".sv_Section").css("font-size", $(".sv_Entry").css("font-size"));

                    $('.items', '#examples').hide();
                    $("#toggle_Examples").click(function() { // TODO: jāvispārina kā atsevišķa f-cija
			...
			$("#twitterButton").click(function() {
				var tweet = 'https://twitter.com/intent/tweet?text=Sk.&url='+encodeURIComponent(location.href)+'&via=AILab_lv';
