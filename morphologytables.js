var MorphologyTables = function() {
    var cases = [['Nominatīvs','Nom.'], ['Ģenitīvs','Ģen.'], ['Datīvs','Dat.'], ['Akuzatīvs','Akuz.'], ['Lokatīvs','Lok.']];
    ...
    function formatInflections(data, paradigm, pluralEntryWord, secondThirdConj) {
        var expand = '<img src="img/expand.svg" id="toggle_Morphology" class="expand_SVG" width="14" height="14" alt="+"/>';
        ...
        if ((paradigm >= 1 && paradigm <= 11) || (paradigm >= 31 && paradigm <= 35) || paradigm == 44 || paradigm == 47 || paradigm == 48) { // Declinable nouns
        ...
        if (paradigm == 13 || paradigm == 14) { // Adjectives
    	...
    	if (paradigm == 30) { // Adjectives
            var morph = '<span class="summary">' + 'substantivizējies īpašības vārds, pamata pakāpe, noteiktā galotne, vīriešu dzimte' + '</span>';
            var table = '<div class="items">' +
    						formatNoun(data, false) +
                            disclaimer +
                        '</div>';
            return morph + ' ' + expand + table;
        }

    	if (paradigm == 40 || paradigm == 41) { // Adjectives
            var morph = '<span class="summary">' + 'substantivizējies īpašības vārds, pamata pakāpe, noteiktā galotne, sieviešu dzimte' + '</span>';
            var table = '<div class="items">' +
    						formatNoun(data, false) +
                            disclaimer +
                        '</div>';
        ...
        if (paradigm >= 15 && paradigm <= 20 || paradigm == 45 || paradigm == 46) { // Verbs, non-reflexive and reflexive
        ...
        if (paradigm >= 22 && paradigm <= 24) { // Numeral
            var noteiktiiba = '';
    ...
    function formatAdjective(data, degree) {
        ...
        for (var def = 0; def < defs_all.length; def++) {
            ...
            for (var gend = 0; gend < genders_all.length; gend++) {
                ...
                result += formatNumCaseTable(words, genders_all[gend][LONG] + ' dzimte', defs_all[def][LONG] + '-' + genders_all[gend][LONG] + '-', tags);
                result += '</div>';
            }
    ...
    function formatNumber(data) {
        ...
        for (var gend = 0; gend < genders_all.length; gend++) {
            ...
            result += '<div style="display:inline-block">';
            result += formatNumCaseTable(words, genders_all[gend][LONG] + ' dzimte', genders_all[gend][LONG] + '-', tags);
            result += '</div>';
    ...
    function formatVerb(data) {
        ...
        for (var i = 0; i < data.length; i++) {
    		 ...
    		 */
    		if (value && !/^.*----.*$/.test(value) && !/^(ne)?vaja(g|dz)(u|i|am|at|ēšu|ēsi|ēsim|ēsiet|ēju|ēji|ējām|ējāt|iet)$/.test(value))
                add_word(words, key, value);
        }
